...
public class WatchServiceImpl implements WatchService {
	...
	public WatchDto save(WatchDto dto) {
		if (dto != null) {
			...
			if (dto.getBrandName().length() >= 3 && dto.getColour().length() >= 2) {
	...
	public boolean create(List<WatchDto> list) {
		if (list != null) {
			for (WatchDto watchDto : list) {
				if (watchDto.getBrandName().length() >= 3 && watchDto.getColour().length() >= 4 && watchDto.getId() >= 0
						&& watchDto.getType() != null) {
					System.out.println("data is valid");
				}
			}
			return true;
